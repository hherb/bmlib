...
| ✅ Done | `parse_json`'s return contract | Issue #33 — annotated `dict \| list`, which is what it always returned, plus an opt-in `require_dict` that raises in `parse_json()` and retries inside `chat_json()` (fail-fast at temperature 0, mirroring truncation). `@overload` on `Literal[True]` keeps a real `dict` for strict callers. Raising unconditionally was rejected: it would hide the fragment loss below rather than fix it, and would lock out the array-shaped agents queued for the port. bmlib's two quality tiers pass `require_dict=True` — a list used to die on `.get()` inside a broad `except Exception` and degrade the paper to UNCLASSIFIED without retrying (unreleased) |
...
| **Quality & maintenance** | | |
| ✅ Done | Test suite | 981 tests + 32 skipped; in-memory SQLite for DB tests, mocked HTTP for API tests, no external services. 30 skips are the PostgreSQL parameterisations of `test_backends.py`, which need `BMLIB_TEST_POSTGRESQL_DSN`; 2 need PyMuPDF |
...
| ⬜ Planned | Cut the next release | `[Unreleased]` has accumulated several bodies of work — PostgreSQL `publications`, PDF→text wiring, body-less JATS, BaseAgent metrics/embeddings, the transparency PubMed step, the `parse_json` contract. Mostly additive, so a minor bump fits; note two behaviour changes that make stored results non-comparable (transparency scores can rise; an unfenced array of objects now extracts whole). `~/src/bmlibrarian` pins `bmlib[ollama]>=0.5.1,<0.6.0`, so 0.6.0 needs that pin lifted downstream |
