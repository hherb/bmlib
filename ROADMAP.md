...
| **Quality & maintenance** | | |
| ✅ Done | Test suite | 953 tests + 32 skipped; in-memory SQLite for DB tests, mocked HTTP for API tests, no external services. 30 skips are the PostgreSQL parameterisations of `test_backends.py`, which need `BMLIB_TEST_POSTGRESQL_DSN`; 2 need PyMuPDF |
