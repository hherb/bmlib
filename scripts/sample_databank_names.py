...
"""Measure PubMed's ``<DataBankName>`` vocabulary against bmlib's two allowlists.
...
registration, and :data:`~bmlib.transparency.analyzer._DEPOSITION_DATABANK_LEVELS`
maps a repository authors deposit *into* to the data-availability level a
deposit there establishes. Both are allowlists, so a name in neither is
credited as neither — deliberately, so that a gap under-credits rather than
scoring an unknown databank as shared data.

Those lists are curated from NLM's published vocabulary, and curation is the
part that goes stale. This script is how the curation stays answerable to the
records, reporting four things per candidate name:
...
2. **The exact spelling in the XML**, which is not always NLM's table spelling.
   The two known-divergent forms of UMIN's registry are both in the set; this
   is what says which one the records actually use.
3. **How bmlib currently classifies it**, so a drift shows up as a line
   reading ``unclassified`` with a non-zero count.
4. **Which level a deposit establishes** — ``full_open`` or, for controlled
   access, ``on_request`` — since that is a mapping, not a membership test.
...
Run it before changing either list:
...
two lists already name. Add a name to ``NLM_DATABANK_NAMES`` when NLM does.
The deliberate exclusions — OMIM, RefSeq, dbSNP, PubChem-*, the UniProt family
— are candidates too: their counts are the evidence for leaving them out, so
dropping them from the table would hide the reason they are not credited.
...
def _candidates(matched: frozenset[str]) -> list[str]:
    ...
    *matched* is every name bmlib matches — the registries and the deposition
    repositories — folded in rather than assumed to be covered by
    ``NLM_DATABANK_NAMES``: a name bmlib matches that the table omits would be
    matched in production and measured by nobody, which is the one blind spot
...
def main() -> int:
    ...
    candidates = _candidates(_TRIAL_REGISTRY_NAMES | frozenset(_DEPOSITION_DATABANK_LEVELS))
    ...
    for name in candidates:
        ...
        deposits = {
            _DEPOSITION_DATABANK_LEVELS[n] for n in lowered if n in _DEPOSITION_DATABANK_LEVELS
        }
        if lowered & _TRIAL_REGISTRY_NAMES:
            kind = "registration"
        elif deposits:
            # The level, not merely membership: this is a mapping, and dbGaP's
            # controlled access establishes `on_request` where the rest
            # establish `full_open`. A mapping that silently changed would not
            # show up in a yes/no column.
            kind = f"data deposition → {', '.join(sorted(deposits))}"
        else:
