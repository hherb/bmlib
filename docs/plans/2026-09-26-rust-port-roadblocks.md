...
**`pdf_converter.py` is ported in full** — `fulltext/pdf_text.rs` carries the pure half (span dominance, one block per line, furniture detection, paragraph reflow and the HTML render; 53 oracle cases) and `fulltext/pdfium_backend.rs` the binding, over `pdfium-render` behind the optional `pdf` feature, with the `FullTextService` adapter. §5 priced it as a ~200-line wrapper; it took 8 tests against real PDFs, and the one measurement worth carrying forward is in that module's docs — **PDFium's `font_weight()` and `font_is_italic()` return nothing usable for a base-14 face**, so bold and italic are derived from the font *name* (verified against PyMuPDF on the same files).
