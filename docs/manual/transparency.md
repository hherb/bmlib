...
**Membership was measured, not chosen.** `industry_funding_detected` feeds a HIGH-risk rule and HIGH applies `tier_downgrade_amount`, so a false positive costs more than a false negative — which is why the change was calibrated against real names sampled from both corpora by `scripts/sample_funder_names.py`: 833 drawn, 816 unique, 417 hand-labelled and committed as `tests/data/funder_names.json`, of which 407 score (the ten ambiguous are excluded). Ten of the 417 labels were corrected against the corpus's own definitions *(unreleased — issue #292)*, bringing the file back to byte-identity with the Swift port's copy: five commercial names had been labelled public-sector (`"Amgen"`, `"AstraZeneca"` — whose `"AstraZeneca."` twin was already `industry` — `"Siemens Healthineers"`, `"PetroChina Major Science and Technology Project"`, `"Lån & Spar"`), and five undecidable ones became `ambiguous` with a reason. No token reaches any of the ten, so every row in `analyzer.py` is unchanged; what moves is recall's denominator, 30 industry names to 35.
