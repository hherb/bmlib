...
**Effect on stored values.** For a well-formed body, only `cited_by_count` moves: a `true` or a fractional count scored 5 points it should not have. For a malformed body, stored values move at every site above — that being the point. Whether any of it fires against the live APIs is now **measured, and on one draw it does not** *(unreleased)*. `scripts/sample_api_failures.py` reads the shape of every 200 body since issue #211, and its first such run (2026-09-08, 124 + 60 records) found every served body well-formed at all five endpoints — 0 non-object, 0 undecodable, 0 empty — with `open_access.is_oa` boolean in 74 of 74, `cited_by_count` a number in 74 of 74, and `hasResults` boolean in 55 of 55. So no coercer above was observed to fire. Read those as upper bounds of a few percent, on a draw that is MED+PPR rather than the declared source spread (issue #212), and note the population is identifiers bmlib is *handed*.
