...
4. **A token is refused where it collides with a form the corpus cannot see, and this rule vetoes the other three.** Two applications: a token of two characters is refused outright, its collision surface being wider than 412 names can sample; a longer token is refused on a *named* collision. The veto is paid in measured true positives and in rule-2 standing, which is why it is a veto rather than a fourth opinion — `"ab"` (Aktiebolag), `"ag"`, `"bv"`, `"nv"` and `"sa"` are every bit as reserved as rule 2's members, and `"co"` carries the strongest corpus evidence of any refused token.
