...
**Effect on stored values.** For a well-formed body, only `cited_by_count` moves: a `true` or a fractional count scored 5 points it should not have. For a malformed body, stored values move at every site above — that being the point. Whether any of it ever fires against the live APIs is **not measured**: nothing in this repo records the *shape* of a 200 body, only its status (issue #211).
