...
**The other failure modes** are treated as ordinary retryable errors:
...
`chat_json()` returns whatever the response parsed to, so a model that answers with a top-level array yields that array **whole**. Callers that need an object pass `require_dict=True` rather than assuming one:

```python
data = agent.chat_json(
    [agent.system_msg("Reply with a JSON object."), agent.user_msg(prompt)],
    require_dict=True,
)
...
|---|---|
| `> 0` | Logged at ERROR, then the normal backoff retry — a resample may answer with the object that was asked for. The final exhaustion error names the shape. |
| exactly `0.0` | Raises `ValueError` **immediately — no retry**. Greedy sampling returns the same array from the same messages, exactly as it reproduces a truncation. |

This mirrors the truncation policy deliberately: two adjacent branches in one method must not sit on opposite rules for the same reason. It matters for a caller pinning temperature 0 for reproducibility — which is exactly the caller most likely to want `require_dict`.
...
| `total_retries` | `int` | Attempts beyond the first, across all `chat_json()` calls. Counted when an attempt *begins*, so a retry whose request then raises is counted without a matching entry in `total_requests`. |
| `total_wall_time_seconds` | `float` | Wall-clock time inside successful requests only, accumulated from `time.monotonic()`. |
| `start_time` | `float \| None` | `time.time()` at the last `mark_start()` / `start_metrics()`, or `None`. An absolute timestamp, for rendering as a date — durations come from `elapsed_time_seconds`. |
| `end_time` | `float \| None` | `time.time()` at the last `mark_end()` / `stop_metrics()`, or `None` while still running. Absolute, as `start_time`. |
...
| `snapshot()` | Return an independent copy, read under the lock, carrying the monotonic marks across so its `elapsed_time_seconds` matches the original's. The only supported way to copy an instance — the `threading.Lock` field makes `copy.deepcopy()` and `pickle` raise `TypeError` directly on a `PerformanceMetrics`. |
...
|----------|---------|-------|
| `elapsed_time_seconds` | Monotonic time since `mark_start()`, frozen at `mark_end()` | Measured on `time.monotonic()`, not on the `time.time()` timestamps in `start_time` / `end_time`. A wall-clock *difference* can be distorted — or made negative — by an NTP step or a DST change mid-run, and `format_report()` prints this figure directly against `total_wall_time_seconds`, which is itself accumulated from `time.monotonic()`; two clocks either side of that comparison is how "12.3s elapsed (14.1s in requests)" gets printed. An instance rebuilt by `from_dict()` has no monotonic marks — they are not meaningful across processes, so they are not serialised — and falls back to `end_time - start_time`. `0.0` if `start_time` is `None`. |
...
|-------|--------|---------------|------------|
| `StudyClassifier` (Tier 2) | `classify(title, abstract)` | constructor defaults `temperature=0.1, max_tokens=1024`; `require_dict=True` | Logs a warning, returns `QualityAssessment.unclassified()` |
| `QualityAgent` (Tier 3) | `assess(title, abstract)` | constructor defaults `temperature=0.2, max_tokens=1024`; `require_dict=True` | Logs a warning, returns `QualityAssessment.unclassified()` |
