...
- **PDF download failure is non-fatal, and now reported (#68).** When a PDF cannot be downloaded or fails magic-byte validation, the result is still returned with `pdf_url` set, so the URL remains a usable fallback; only `file_path` is left unset. If a body-less JATS document was seen earlier in the chain, its abstract is merged into that result rather than discarded, so the caller gets an abstract plus a link instead of a bare link. Three distinct outcomes, previously all silent below `DEBUG`, are now told apart:

  ```
  Could not download a europepmc PDF (HTTP 404; first seen at https://europepmc.org/articles/PMC.../bin/....pdf?pdf=render). The URL is left on the result, but there is no file and no extracted text. This is common enough that it is reported once — run with DEBUG logging to see every affected article.

  Could not download a unpaywall PDF (the response is not a PDF; first seen at https://.../landing-page). The URL is left on the result, but there is no file and no extracted text. This is common enough that it is reported once — run with DEBUG logging to see every affected article.
