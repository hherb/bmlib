...
Tier 2 runs by default when Tier 1's metadata result is not confident enough (`use_llm_classification=True`); Tiers 3 and 4 are opt-in via [`QualityFilter`](#qualityfilter). Tier 3 supersedes every shallower tier when requested, rather than running alongside it; Tier 4 supersedes Tier 3, but still needs a classification tier to supply the `study_design` it does not produce itself — see [Cochrane assessment](#cochrane-assessment). Alongside the pipeline, the module ships two toolkits that remain **standalone**: Cochrane table formatters, and rule-based (LLM-free) extractors with an audit-trail scoring model.
...
| `use_cochrane_assessment` | `bool` | `False` | Enable Tier 4 (Cochrane assessment). Supersedes Tier 3 when both are set, and runs Tier 2 first when Tier 1 was inconclusive, for the `study_design` it does not produce itself — see [Cochrane assessment](#cochrane-assessment). |
...
4. **Tier 4 (Cochrane assessment):** if `use_cochrane_assessment` is `True`, `CochraneAssessor.assess()` runs against `full_text` (falling back to `abstract` when `full_text` is absent). **On success** it *enriches* a classification rather than replacing it — see [Cochrane assessment](#cochrane-assessment) for what that means — and **Tier 3 is skipped entirely**, since Tier 4 is deeper than Tier 3 exactly as Tier 3 is deeper than Tier 2. **Tier 2 is skipped only when Tier 1 was confident.** A Cochrane assessment produces no `study_design`, so the result it enriches has to carry one: when Tier 1 was inconclusive and `use_llm_classification` is `True` (the default), `StudyClassifier.classify()` runs and *its* result is what the Cochrane data is merged into. That case is not an edge — a bioRxiv or medRxiv record has no PubMed publication types at all, so Tier 1 is inconclusive for exactly the papers whose full text makes a Cochrane pass worth paying for. With `use_llm_classification=False` the caller has declined the cheap tier, and the (unclassified) Tier 1 result is enriched as-is. **On failure** (`CochraneAssessor.assess()` returns `None`, e.g. a transport error) there is no result to supersede anything with, so execution falls through to step 5 exactly as if `use_cochrane_assessment` had not been set — a failed Cochrane pass must not cancel the Tier 3 assessment a caller separately asked for.
...
A *successful* Tier 4 pass never runs alongside Tier 3 in the same call, and runs alongside Tier 2 only to obtain the `study_design` it cannot produce itself. Tiers 2 and 3 *replace* the Tier 1 result outright; a successful Tier 4 *merges* into whichever classification it has (see below) — the one tier that does not discard the classification it followed. A **failed** Tier 4 pass produced nothing to merge, so it is not a fourth tier that ran and lost: the pipeline falls through and Tier 3 or Tier 2 may still run in that same call, exactly as they would have without `use_cochrane_assessment` set.
...
*(unreleased)* `from_dict()` requires no key (#310): a missing or unreadable `methods` reads `"Not reported"`, and a missing section reads as that section's own defaults. The identity fields are the caller's and are read verbatim, `study_id` included, which defaults to `"Not reported"` only when absent or `null`. The exception is `created_at`: one that is not an ISO 8601 string reads as absent, so `__post_init__` stamps the current time, rather than raising.
...
`QualityFilter(use_cochrane_assessment=True)` routes `QualityManager.assess()` through `CochraneAssessor` instead of Tier 3 — which is skipped entirely, exactly as Tier 3 already skips Tier 2 when requested. `QualityManager.assess()` gained a `full_text: str | None = None` keyword for this: the Cochrane pass reads `full_text`, falling back to `abstract` when it is absent (a weak risk-of-bias assessment beats none).

The Cochrane pass **enriches** a classification rather than replacing it: the classification supplies `study_design`, `quality_tier`, `quality_score` and `confidence`, which a Cochrane assessment does not produce; the Cochrane pass supplies `bias_risk` (via `collapse_risk_of_bias()`) and the full `cochrane_assessment` object, which no classification tier can see. On success the returned `QualityAssessment` carries `assessment_tier=4`, `extraction_method="llm_cochrane_assessment"`, and `cochrane_assessment` set to the `CochraneStudyAssessment`.

**Which classification it enriches depends on Tier 1.** A confident Tier 1 result is the base and Tier 2 is skipped. An inconclusive one is not: because a Cochrane assessment produces no `study_design` of its own, enriching an unclassified Tier 1 result would return `study_design=UNKNOWN` at `quality_score=0.0` and `confidence=0.0` with a full nine-domain bias table attached — worse than the Tier 2 answer the caller had enabled. So when Tier 1 is inconclusive and `use_llm_classification` is `True` (the default), the cheap classifier runs first and its result is the base. This is the common path for preprints: a bioRxiv or medRxiv record has no PubMed publication types at all, so Tier 1 is inconclusive for exactly the papers whose full text makes a Cochrane pass worth paying for. With `use_llm_classification=False` the caller has declined that tier, and the unclassified Tier 1 result is enriched as-is. Tier 3 is superseded either way — it is the expensive tier whose work the Cochrane pass actually replaces.

Two fields are deliberately **not** copied across. `evidence_level` is free-form model text on the Cochrane side against an Oxford CEBM level on the classification side — different vocabularies. `confidence` is not touched either: `CochraneStudyAssessment.overall_confidence` describes how sure the model was about the nine bias-risk domains, not about the `study_design` / `quality_tier` / `quality_score` that still come from the classification after enrichment, so overwriting `confidence` with it would let a caller's `if a.confidence >= t: trust a.study_design` pattern discard a highly-confident classification because the model was merely middling-sure about blinding. Both values stay reachable at `result.cochrane_assessment.evidence_level` and `result.cochrane_assessment.overall_confidence`.
...
result.study_design           # from Tier 1, or Tier 2 when Tier 1 was inconclusive
