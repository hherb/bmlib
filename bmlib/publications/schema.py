...
Creates tables for publications, full-text sources, download tracking, and
retraction notices on either backend.  The two DDL strings differ only where
the dialects do: surrogate keys (``AUTOINCREMENT`` vs ``SERIAL``) and booleans
(SQLite has none).  Everything the storage layer leans on — the partial unique
indexes on ``doi``/``pmid`` that make cross-source deduplication work, and the
``UNIQUE`` constraints backing ``ON CONFLICT`` — exists in both.
"""
...
# ``CREATE TABLE IF NOT EXISTS`` never rewrites a table that already exists —
# so switching would not migrate one existing database, it would only make new
# ones differ from every database already in the field. Divergence with no
# behavioural gain is the wrong trade for a library several projects depend on.
...
#
# Deliberately not :func:`bmlib.db.run_migrations`. That runner records applied
...
# with the database's actual state. The cost is one catalogue query per
# :func:`ensure_schema` call; if this list ever grows past a handful of
...
def _existing_columns(conn: Any, table: str) -> set[str]:
    ...
    about some *other* database's ``publications`` table: a second consumer in
    its own schema, upgraded at a different time, is enough to report a column
    as present that this schema does not have. The ALTER would then be skipped
...
def ensure_schema(conn: Any) -> None:
    ...
    **Call this once after upgrading bmlib.** Reads tolerate a database that
    has not been through it — :func:`bmlib.publications.storage` treats a
    post-release column as absent rather than raising — but writes do not:
    :func:`~bmlib.publications.storage.store_publication` names every column in
    its INSERT and will fail on one the database lacks.
    :func:`bmlib.publications.sync` calls this for you.
    """
