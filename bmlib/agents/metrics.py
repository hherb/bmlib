...
class PerformanceMetrics:
    ...
    Attributes:
        ...
        total_retries: Attempts beyond the first, across all requests.  Counted
            when an attempt *begins*, so a retry whose request then raises is
            counted without a matching entry in ``total_requests``.
        total_wall_time_seconds: Wall-clock time inside successful requests.
        start_time: When collection started, or ``None``.  A :func:`time.time`
            timestamp, so it can be rendered as a date; durations come from
            :attr:`elapsed_time_seconds` instead.
        end_time: When collection ended, or ``None`` if still running.  A
            :func:`time.time` timestamp, as :attr:`start_time`.
    """
    ...
    # Monotonic counterparts of start_time/end_time, backing
    # elapsed_time_seconds.  Not serialised: a monotonic reading is only
    # comparable to others from the same process, so carrying it through
    # to_dict() would be meaningless to whoever reads it back.
    _monotonic_start: float | None = field(default=None, init=False, repr=False, compare=False)
    _monotonic_end: float | None = field(default=None, init=False, repr=False, compare=False)
    ...
    def snapshot(self) -> PerformanceMetrics:
        ...
        with self._lock:
            ...
            # init=False fields, so they have to be carried over by hand —
            # otherwise a snapshot's elapsed time silently drops to the
            # wall-clock fallback.
            copy._monotonic_start = self._monotonic_start
            copy._monotonic_end = self._monotonic_end
        return copy
    ...
    def elapsed_time_seconds(self) -> float:
        ...
        Measured on :func:`time.monotonic`, while :attr:`start_time` and
        :attr:`end_time` are :func:`time.time` timestamps.  The split is
        deliberate: those two are absolute and a caller may want to render
        them as dates, but a wall-clock *difference* can be distorted — or
        made negative — by an NTP step or a DST change mid-run, and
        :meth:`format_report` prints this figure directly against
        :attr:`total_wall_time_seconds`, which :class:`BaseAgent
        <bmlib.agents.BaseAgent>` accumulates from :func:`time.monotonic`.
        Two clocks either side of that comparison is how "12.3s elapsed
        (14.1s in requests)" gets printed.

        An instance rebuilt by :meth:`from_dict` has no monotonic marks —
        they are not serialisable across processes — and falls back to the
        ...
        if self._monotonic_start is not None:
            ...
            return end - self._monotonic_start

        if self.start_time is None:
            return 0.0
        end_wall = self.end_time if self.end_time is not None else time.time()
        return end_wall - self.start_time
