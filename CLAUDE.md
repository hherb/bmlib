...
And **an exhibit is a stack, its content goes to the innermost one, and a footnote's `<label>` is not its number**. `<fig>` and `<table-wrap>` both nest — eLife wraps every figure supplement inside the figure it belongs to (19.6% of 225 surveyed articles; PMC8754430 returned 9 of 12), and JATS lets a `<table-wrap>` open inside another's `<table-wrap-foot>` — so each is a stack of `_ExhibitFrame` and never a single slot, which the inner open overwrote and the inner close emitted and cleared, losing the parent outright (issue #115). Two halves, and a test that only counts exhibits catches neither: `in_figure`/`current_figure`/`in_table_wrap`/`current_table` are **derived** from the stacks rather than stored, since the flag the inner close cleared is what read the rest of the parent as article prose; and the list entry is **reserved when the element opens and filled when it closes**, since an exhibit is built at its end tag but has to be listed at its start — pop-and-append restores the parent and still lists every supplement ahead of it. Routing is to the **innermost** open exhibit (`_innermost_exhibit`, the one whose `open_seq` is highest), never "the figure if any is open, else the table": exhibits nest both ways round, and the ambient-flag order handed an inner table's own label and caption to the figure enclosing it. A `<label>` inside a footnote is that footnote's marker and not the exhibit's number (issue #116 — 12.0% of articles carry a labelled `<table-wrap-foot><fn>`, and the last marker won, so PMC12661592's one table was labelled `a`); the footnote depth is compared against the depth **that exhibit** opened at and never against zero, because JATS lets a `<fig>` or `<table-wrap>` open inside a footnote and the zero test eats *its* label. `_FOOTNOTE_CONTAINERS` earns `fn` from the measurement and `table-wrap-foot` as the container covering a `<fn-group>`'s own heading. The marker is discarded rather than held only because no footnote prose is captured to attach it to — that is issue #124, and #123 is the nested `<caption>`, which is this same family and still open. Among several `<graphic>`, the deposits are **ranked** (`ARCHIVAL < THUMBNAIL < FULL`) and one is accepted only when **strictly** better, so the first wins among equals: 58.0% of 959 figures carry more than one and 52.9% end on a thumbnail, so "keep the last" resolved most figures to a preview — while "keep the first" only trades it for an `<alternatives>` TIFF, the two conventions depositing at opposite ends (issue #117). `thumb` is matched as a lowercased substring of `content-type` **or** `specific-use`, and **nothing is inferred from the file extension**: every corpus thumbnail is a `.gif` because PLOS and Springer both deposit that way, so an extension rule passes the corpus and then discards the only image a figure has elsewhere.
