...
**Both committed corpora were redrawn for #138, and the redraw is what the figures below are of.** Each is 997 articles whose *identifiers* come deterministically from a named public artifact — `oa_comm_xml.PMC012xxxxxx.baseline.2025-06-26.tar.gz` for 2023-2025 and `oa_comm_xml.PMC002xxxxxx.baseline.2025-06-26.tar.gz` for 1996-1998, at `seed 0` — while the *bytes measured* are Europe PMC's `fullTextXML`, the rendition `FullTextService` feeds the parser. The two halves are separate because the two renditions disagree on the cited populations and not merely on detail: over 294 shared identifiers `last_is_thumb` measures **0** on archive bytes against **641** on served ones, the archive depositing one bare `<graphic xlink:href="…-g001">` per figure where Europe PMC synthesises an `.jpg`/`.gif` image-and-thumb pair — so a corpus drawn *and* measured from a baseline package would put #117's whole ranking rule out of reach and read it as dead code. `tests/data/jats_exhibits.rendition.json` is that comparison. Every figure below moves for two reasons at once, the sample and the rendition both being different, so **no movement here may be attributed to the walk's scoping alone**.
