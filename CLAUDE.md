...
- **`transparency/`** — Queries CrossRef, Europe PMC (search + full text), PubMed, OpenAlex, and ClinicalTrials.gov to compute a transparency score (0-100) covering funding, COI, data availability, trial registration, and open access. The PubMed step is one `efetch` per analysis, skipped without a PMID (taken from the caller or from the Europe PMC record already fetched); it supplies structured `<CoiStatement>`, `<DataBankList>` and `<GrantList>` signals that Europe PMC cannot give for a closed-access paper, and `pubmed_api_key` rides on it. `<DataBankList>` is read twice over, against two disjoint allowlists: a trial-registry name establishes registration, and a public-archive name (GENBANK, PDB, figshare, Dryad, GEO, SRA, …) is a data deposition that sets `data_availability_level` to `full_open`. A name in neither is credited as neither — reading the registry set's complement as archives would score an unlisted registry as open data, and would credit the curated databases (RefSeq, OMIM, SWISSPROT) an author cannot deposit into. Both sets are measured with `scripts/sample_databank_names.py`. That field therefore has two producers, so it is written through `_Analysis.note_data_availability()` — strongest evidence of openness wins whichever arrives first, the superseded level's credit is taken back rather than added to, and `unknown` never displaces a finding. When no API is reachable the result is `UNKNOWN` at score 0, so an unreachable network does not masquerade as a HIGH-risk paper; `TransparencyResult.unknown_reason` says which of the three `UNKNOWN` cases it was, set if and only if `risk_level` is `UNKNOWN`.
