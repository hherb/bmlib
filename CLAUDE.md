...
- **`transparency/`** — Queries CrossRef, Europe PMC (search + full text), PubMed, OpenAlex, and ClinicalTrials.gov to compute a transparency score (0-100) covering funding, COI, data availability, trial registration, and open access. The PubMed step is one `efetch` per analysis, skipped without a PMID (taken from the caller or from the Europe PMC record already fetched); it supplies structured `<CoiStatement>`, `<DataBankList>` and `<GrantList>` signals that Europe PMC cannot give for a closed-access paper, and `pubmed_api_key` rides on it. When no API is reachable the result is `UNKNOWN` at score 0, so an unreachable network does not masquerade as a HIGH-risk paper; `TransparencyResult.unknown_reason` says which of the three `UNKNOWN` cases it was, set if and only if `risk_level` is `UNKNOWN`. **An article's full text is addressed by its Europe PMC accession alone** (#184): the URL carried a `{source}/` segment, which Europe PMC answers with its own 404 (their CORS headers, `content-length: 0`), so **every** fetch failed and the module scored every open-access paper on its abstract — up to 30 points off, `coi_disclosed` unable to reach `False`, and silent, a non-200 being the one outcome that deliberately does not warn. Measured, not read off the docs: the single-segment form serves 200 for four PMC ids spanning 2012-2025 and six `PPR` accessions, while `{source}/{ext_id}`, the bare numeric id and the PMID all 404, as do the sibling `textMinedTerms` and `supplementaryFiles` — the path *shape*, not one endpoint. The accession is **not a PMCID and must not be normalised into one**: 75,760 of Europe PMC's 12,220,678 `IN_EPMC:Y` records (0.62%, their own hit counts rather than a draw) are preprints carrying no `pmcid`, so `fulltext/service.py`'s `_normalise_pmc_id` would reject every one of them — the two modules agree on the *base* and deliberately not on the identifier — by assertion, each keeping its own literal, since importing across would be the runtime dependency `transparency` does not have; `EUROPEPMC_REST_BASE` unified this module's **own** two literals, the search call and the full-text call, which is where the drift was. The guard beside it dropped its `source` half at the same time: it addressed nothing any more, and a guard kept past its reason refuses a fetch that works. **The issue's own remedy for the silence was refuted** — it proposed raising the non-200 log level on the argument that a 404 under `inEPMC: Y` is Europe PMC contradicting itself, but `inEPMC` says they *hold* the text where `fullTextXML` serves the open-access subset: of 150 `IN_EPMC:Y` records stratified by source and year, **0 of 53** `isOpenAccess: N` records served and 35 of 97 open-access ones still 404'd, so a WARNING would fire on every closed-access paper analysed. It logs the **URL** at DEBUG instead, since the URL is what named the defect — but what catches the next one is the test, not the line: `_FakeFullTextClient` matched `url.endswith("/fullTextXML")` and `_RecordingClient` — the `analyze()`-level fake — the looser `"fullTextXML" in url`, and both forms satisfy either, so **236 of `main`'s 236 tests passed with the defect in place**; matching the whole URL in both reddens **52 of this branch's 249, 43 of them pre-existing**, which is the `parser_log` fixture's trick one module over — a net that is free because existing tests become URL checks without being rewritten. Restoring full text moves stored values broadly; the diff is in `CHANGELOG.md`. A record carrying no *PMC* accession is still fetched by PMID — a record carrying no accession at all is refused by the guard and reads `NOT_ATTEMPTED` — and still stores `NOT_SERVED` for the guaranteed 404 — filed as #188 with the closed-access population beside it, both being requests whose outcome is known in advance. **The full text it scans is the article's own, and the module never sees `JATSParser` output** (#119): it fetches `fullTextXML` and regexes the raw string, so `_fetch_europepmc_fulltext` removes every `<sub-article>`/`<response>` region before returning — one door, so the tagged-COI match, the cue-phrase scan, the data-availability patterns and the industry-COI extraction all read a string that has to be the article's. Reviewers write in exactly the vocabulary these scans hunt for, and a round's *"the reviewers declare no competing interests"* was the paper's disclosure. The two-element set and its completeness argument are `jats_parser`'s, **restated rather than imported** so `transparency` needs nothing from `fulltext`. A **depth**, not a flag, since JATS nests them; and because a literal `<` can only open markup in well-formed XML, a comment, a CDATA section, a processing instruction and the DOCTYPE internal subset are the *complete* set of places `<sub-article` is not a start tag — all four are lexed as tokens, which is what makes the scan exact rather than a list of hazards someone thought of (the doctype's own fixture was vacuous until a mutant said so). An **unclosed region returns `None`** and the analysis falls back to the abstract: scanning the tail is the defect, and dropping it silently manufactures *"No COI disclosure found in full text"*, the finding that (absent a PubMed `<CoiStatement>` to rescue it) triggers the missing-COI downgrade — while an unmatched *end* tag **at depth 0** is not an imbalance in that sense and costs the article nothing. An unmatched end tag *inside* a region names an element that did not open it, so it closes nothing and the region stays open to be refused — the regions are a **stack of names**, since a bare depth let `</response>` close one a `<sub-article>` had opened and re-admit the outer round, which is #119's own defect reached through the fix for it (#160). Only a document expat would reject can carry either. A **fifth lexer branch** matches a bare opener and refuses a construct that never terminates, which is what bounds the work: the four skip branches each scanned to end-of-string with the scan retrying at every later opener, so the lex was quadratic (33.6s for 256 kB of `<!DOCTYPE a[`, 33.3s for 224 kB of an unterminated tag, each doubling costing about four times the last, against 4-9 ms for the corpus's three largest well-formed articles) and the HTTP timeout bounds the request, not this — a truncated body did not fail, it stalled, and its own content was read as markup. Refusing at the first opener costs one failed scan and stops, and needs no constant, unlike the size cap and work multiple the issue weighed against it. It raises rather than returning `None` because "will not segment" and "did not arrive" are different claims (#161's shape one level down), and its `<` sits **outside** the capturing group: `sre` derives a prefix only when every top-level branch opens with the same literal. Three configurations over 7.8 MB, and they must be named or two get conflated — 13.4 ms with no refusal branch, 26.6 ms with it and the literal outside, 191 ms with it inside — so the *placement* penalty is 7.2x and the guard's own cost is 1.9x; the "14x" a first draft carried multiplied the two by comparing the misplaced guard against no guard at all. Pinned by `test_every_branch_of_the_lexer_opens_with_the_literal`, and re-measured on a re-derivable draw (the first 880 articles of `PMC10030002_PMC10040000.xml.gz`, 90.8 MB served, nine interleaved runs) at 1.80x and 6.6x — interleaved because run-to-run drift reached 27%, wider than the gap between two of the ratios. Both fixes need input expat would reject, and lexing both corpora with and without them moves **0 of 98,789** articles. **A body truncated *between* tags is the shape none of that reaches** (#183): it opens no unterminated construct, leaves no region open and empties nothing, so it used to be scanned as a complete article and yield `coi_disclosed=False` — the missing-COI downgrade on evidence that does not exist, and silent where #160's half is loud. A fourth refusal now tests the served body for `</article>`, and **presence, not position, is the measured choice**: the issue's own `rstrip().endswith(...)` refuses complete articles, since trailing comments, PIs and whitespace after the root are legal and 1,727 of 97,909 archive articles (1.76%) and 23 of 8,118 served ones (0.28%) end `</article><!--requester-ID …-->` — both honest on the bundle, measuring the gap between one end tag and the next opener. **The presence test's own 0 is of the archive half alone** (0 of 97,909 individual documents): the served bundle is one concatenation split on `</article>`, so containment there is true by construction, and the "0 of all 106,027" a first cut carried pooled a tautology into the evidence. A truncation removes the tail and the root's end tag is *in* the tail; `</sub-article>` does not contain the substring, so the strip cannot affect it. **It runs last of the four and the order is load-bearing** — a truncated body satisfies several at once, truncation being the cause and the rest symptoms, and each of the others knows something it does not: ahead of the lex it makes #160's construct-and-offset message unreachable for the input that most often produces it (a *corrupted* body still carries `</article>` and reaches the lex, so "only" overstates it), and ahead of the entirely-nested report it makes that unreachable too, since a body of nothing but `<sub-article>` carries no `</article>` either. The unclosed-region refusal knows *which* element too and discards it at the return — #186, and the ordering argument does not rest on it. Whether Europe PMC ever serves a legitimately partial body at HTTP 200 is **not measured**, and the local draw is not evidence of it — that importer may have discarded failures. **Every refusal now leaves a trace in the stored result** (#161): all of them used to reach storage as `full_text_analyzed=False` plus *"full text unavailable"*, which is false for a document served at HTTP 200 and refused, while the score lost up to 30 points — enough to reach HIGH against the default threshold. `FullTextStatus` on `TransparencyResult` is the machine-readable form and `is_refusal` the grouping, so no call site enumerates the members and one added later must choose a side — **mechanised, not asserted**: both sides are named sets and `test_every_status_chooses_a_side` asserts the partition, since an eighth member omitted from the refused set defaults to *not a refusal* and routes into the "full text unavailable" indicator, so the silent default runs the wrong way (`TestTheAuditNetIsComplete`'s rule, one type over). It mirrors `unknown_reason` throughout, and **`None` means *not recorded*, never `NOT_ATTEMPTED`**, since a legacy result may carry `full_text_analyzed=True` and a determinate reading of that is the worse answer — so **no path this version writes leaves it `None`**, the three early returns included, or a current row is indistinguishable from a legacy one. `__post_init__` enforces the one direction it can. The prose half is retracted with the other two COI lines when PubMed supplies a `<CoiStatement>`: `_INDICATORS_RETRACTED_BY_PUBMED_COI` names all three in one place, the third having been added to the appending site and not the retracting one, which stored *"status unknown"* beside *"disclosure found"* — #161's own failure mode inside its fix. Same argument as `FetchResult.note` → `SyncReport.notes` one module over. Measured over PMC's `oa_comm` baseline package `PMC012xxxxxx` (2025-06-26, 97,909 articles): 3,382 (3.45%) carry a region it removes — 3,377 a `<sub-article>`, 5 more a top-level `<response>` and no `<sub-article>` — and 602 of those, 0.61% of the corpus, have a scan output move (499 the data level, 125 the COI cue with 4 flipping the stored tri-state, 6 the industry-COI signal — `industry_funding` and `industry_confidence`, not only the indicator string — 1 the tagged section). None of the five `<response>` articles is among the 602, so that element is **rare, not absent** — the first cut of this claim said it measured zero, which was a grep for the other element. Nesting is exercised rather than defensive: 98 of the 3,382 carriers nest. Two populations measure **empty** and say so: no article leaves a region open (0 of 97,909), and none is emptied by the removal (all 3,389 carriers keep their `<body>`, the least retaining 32.2% of its bytes). **The lexer's four skip tokens have no measured population on this module's own input**: the comment token fires on 3 archive deposits — Springer commenting out an `<authorqueries>` block whose `<aq>` children carry `<response>` — but Europe PMC's `fullTextXML`, which is what the module reads, serves those same three with no comments at all and carries one in 0 of an 880-article draw against 25.6% of the archive. They are kept for the structural argument, not for a population. **Industry-funder membership follows four rules, and rule 4 vetoes the other three** (#112): corpus evidence earns a token; a reserved incorporation suffix is a **prior, not proof**, so it is kept where `tests/data/funder_names.json` is silent (`corp`, `gmbh`, and now `plc`/`pty`); the residue of a disqualified stem is kept as a bare word where it cannot match more than the stem it replaced (`pharma`, `biotech`, which satisfy neither of the first two — the category that went unnamed while the block claimed to cover every token); and a token colliding with something the corpus cannot see is refused even where it passes the count (`ab`, `labs`, and `co`, which measures 4 TP / **0** FP and whose recorded collision is in no corpus entry). Stating one rule and applying another is what left `plc`/`pty` out on a score four kept tokens share. Rule 2 is only a prior because the older premise — *no public body may use the form* — is false: `Forschungszentrum Jülich GmbH` and `Genome Research Limited` are flagged today, and the corpus's own single GmbH is ambiguous-labelled as academic, so that row's 0 TP / 0 FP means *not scored*, never *not present* (#156). `plc` is the one member rule 4 reaches and does not refuse — PLC is also *phospholipase C* — kept on a stated distinction and measured by #157. **Every row is an input under test, and not only its counts**: `TestTheStatedCountsAreWhatTheCorpusHolds` parses the rows out of `analyzer.py` itself, and the headline table out of `docs/manual/transparency.md`, and re-derives each row's counts, its `in`/`out` against the tuples, and the rule it cites — because arithmetic was never the defect, and the first cut stayed green while a row was moved into the refused block with its token still in `_INDUSTRY_WORDS`. It asserts the corpus's own size too, every count being a numerator over it. Known false positives are pinned by `TestTheKnownFalsePositivesAreKnown` rather than described. Eight claims were stale before it existed — seven figures and one named example — and not by drift: one corpus commit, a byte-identical matcher, and a set of numbers self-consistent with a 34-industry revision never committed (the committed one holds 30). It fails closed — an unreadable source, a table reformatted out of recognition and a doubled claim all raise, because "I found nothing" must not be an answer a net can return.
...
**Both committed corpora were redrawn for #138, and the redraw is what the figures below are of.** Each is a 1,000-article draw — 997 of the recent one and all 1,000 of the back-filled one served — whose *identifiers* come deterministically from a named public artifact — `oa_comm_xml.PMC012xxxxxx.baseline.2025-06-26.tar.gz` for 2023-2025 and `oa_comm_xml.PMC002xxxxxx.baseline.2025-06-26.tar.gz` for 1996-1998, at `seed 0` — while the *bytes measured* are Europe PMC's `fullTextXML`, the rendition `FullTextService` feeds the parser. The two halves are separate because the two renditions disagree on the cited populations and not merely on detail: `last_is_thumb` **differs in 156 of 300 compared articles, and where it differs the archive measures 0 against 781 served** — so a corpus drawn *and* measured from a baseline package would put #117's whole ranking rule out of reach and read it as dead code. **Read that at that scope and no further**, which is this repo's own rule about a count being of what you looked for, got wrong first by the person writing the rule down: `tests/data/jats_exhibits.rendition.json` records a field only where the renditions disagree, so an agreeing article never appears and the archive's total over all 300 is not derivable from it. Nor is there one mechanism to name — an early draft said the archive deposits a bare `…-g001` per figure where Europe PMC synthesises an `.jpg`/`.gif` pair, true of a spot-checked article and false in general, since `PMC12169732` deposits its own four thumbnails as `specific-use="thumbnail"` where Europe PMC re-labels them `content-type="thumb"` and both measure four. That article was drawn out of the held sample by the second redraw, so it is a **live spot-check** (re-run 2026-09-02) and not a row of the artifact — which is exactly why the scope caveat is structural rather than resting on it. Every figure below moves for **three** reasons at once — a different sample, a different rendition, and a scoped walk — so **no movement here may be attributed to any one of them**, the scoping least of all, since it is the only one whose effect the corpus records (`unscoped`).

One rule measures **empty**, and the comments say so rather than implying a population: of **7,055** `<graphic>` inside an `<alternatives>` across the two draws, **none declares a `mime-subtype` and none is archival by either test**, so the whole `ARCHIVAL` rank is unreached. It is kept because what it prevents is silent and permanent, and "no instance" is not "cannot happen"; the `<p>` member of the transparent set is *not* defensive, since without it a figure whose graphic sits in prose flow loses it outright. **The second one moved twice, and matters more than it did.** A vanished 276-article draw found exactly one `<graphic>` owned by a non-exhibit inside an exhibit, which read as a population of one; the recent committed draw finds **153, in 12 of 997 articles, over three owners** — `<td>` 82 (8 articles), `<inline-formula>` 69 (3), `<disp-formula>` 2 (1) — and since #127 gave `JATSTableInfo` a `graphic_url`, relaxing ownership lands a cell decoration in it as if it were the table's own rendition. So that rule is measured as load-bearing, not carried against a hypothetical, and the owner spread is itself the argument for keeping the *listed* side short and everything else opaque: the previous draw of this same window turned up `<chem-struct>` and `<th>` as well, which this one does not, so the owners are drawn from rather than fixed. **The parent rule's premise is neither refuted nor confirmed by the committed corpus, and the claim that it was refuted is #162.** That claim read `exhibits_with_descendant_label` — an exhibit holding **any** `<label>` in its subtree — as "carries its own label indirectly", so 6,937 direct against 6,944 printed as `PREMISE VIOLATED`. The difference is the set a descendant-search fallback would *fire* on, and nothing more: fetched from Europe PMC (2026-09-02), all seven of those exhibits are a `<table-wrap>` carrying no `<label>` **and no `<caption>`**, every label below them being a `<table-wrap-foot><fn>` marker (`*`, `**`, the empty string) or a `<list-item>` bullet in a cell (`1.`, `-`, `•`) — the two containers #116 was about, so the proposed fallback would have corrupted **7 of 7**. Four carry ids their publisher reserves for an unnumbered table (`array1`, `array2`, `utbl0001`). Deciding the premise needs a rule for which descendant label *would* have been the exhibit's own, which is the rule under test, so it stands on its argument and not on a draw — **a count is of what you looked for, one more time, inside the instrument built to check it**. The honest population beside it is a different one and is the reason #162 changed behaviour: **121 exhibits of 7,058, in 83 of 997 recent articles (1.7% and 8.3%), carry no `<label>` of their own**, and `to_html` gave each an invented `Figure {i + 1}` / `Table {i + 1}` — #116's own symptom from the other side, and worse than a blank for #116's own reason, since the invented number is the *index* and so collides with a real one. It renders no number now, and no `alt` and no empty `<figcaption>` built from one. `print_report` lost the `PREMISE VIOLATED` half of its verdict and **kept the other**: `direct` is a subset of `descendant` by construction, so a zero difference does prove no exhibit carries its label indirectly, and dropping both left the report with no content-level line that changes between draws. It is phrased as what was measured, and guarded against its own vacuity — a draw carrying no `<label>` (or no `<caption>`, which is the back-filled window, 0 of 627) satisfies the zero test with an absent denominator, reported as `NO POPULATION HERE` rather than borrowing the `NOT MEASURED` reserved for a missing counter generation. The parent rule is also much the better of the two on the one comparison the corpus does support: a depth counter would *mis-assign* **561 labels in 95 of these 997 articles** — `<fn>` 330, `<list-item>` 225 and `<supplementary-material>` 6, the last a container no enumeration written for #116 named — and a corruption is worse than a blank. The back-filled window numbers every exhibit it deposits — 627 of 627, all `<fig>` — so it contributes to neither population. Re-measuring #117 on the two draws gives **57.8%** [56.3-59.2] of 4,602 recent figures carrying several graphics and **57.3%** [55.9-58.8] ending on a thumbnail, and **44.0%** on both counts for 627 back-filled ones, against the 58.0% / 52.9% of the original 225-article survey and the 49.9% / 49.5% of the vanished 276-article draw — and **0%** depositing a thumbnail first in every one of them. Those are the **owner-scoped** counts since #164: the sampler walked a whole subtree until then, so a `<td>`'s cell image and a nested supplement's deposit counted as the enclosing figure's. Scoping it moves **one** of the four counters by **18 figures** — 2,676 to 2,658, 58.1% to 57.8%, each inside the other's interval — and nothing else, in either window; the corpus keeps both readings per row so the correction is derivable from one file. **That total is not diffuse and the report says so**: the 18 sit in **4 articles of 997**, each losing *every* multi-graphic figure it had (three to `<inline-formula>` images in captions, one to eLife's nested supplements), which is a per-publisher property in the way this module's nesting rate is. **The size of that answers #164 against its own expectation.** Its archive spot-check moved the count 77 to 58 and read as large — an ad-hoc draw over package bytes that is in the repo nowhere, so it is quoted like the 225- and 276-article draws and not as evidence a reader can re-take; the absolute correction is the same either way (19 there, 18 here), but the archive holds 77 multi-graphic figures against the served rendition's 2,676, so one denominator makes it a quarter of the population and the other two thirds of one percent — *a share is of a denominator, and the rendition chooses the denominator*.

And **a `<title>` and a `<caption>` are routed by their owner too** (issues #125, #130, #123), which is the same rule one element further out. `<sec>` is far from the only element carrying a `<title>`: `<fn-group>` is modelled `(label?, title?, (fn|p)+)`, and `<ref-list>`, `<glossary>`, `<app>`, `<boxed-text>` and every `<caption>` carry one, so asking only "is a section open?" let any of them rename it — leaving not a blank but a heading the publisher never wrote, which is why it survived. **Measured, and not small**: 411 titles in 104 of 997 recent articles (10.4% [8.7-12.5]), owned by a `<caption>` (387, in 94 articles), a **`<def-list>`** (12, in 12) and an `<fn-group>` (12, in 3) — what owns that `<caption>` is not recorded, the sampler counting the `<title>`'s immediate parent alone. The `<def-list>` is the parent test's whole argument restated: every redraw has turned up an owner neither issue names, the previous back-filled draw's being a **`<list>`**, and no enumeration written from #125 and #130 would have held either. #125's own `<fn-group>` shape is in *this* draw, where it was in neither of the two before it, and also reproduces on eLife's PMC8754430; 12 titles in 3 articles is a floor for that shape rather than a rate. The redrawn back-filled window carries no renaming title at all, holding no `<caption>` whatever, so the whole population now rests on the recent draw. A `<caption>`'s prose is a **stack of owners**: held as a stored boolean, the inner `</caption>` truncated the enclosing caption at the point the inner one ended, and a depth fixes only that half, since the inner legend's owner is not an exhibit bmlib models. **Neither of that half's populations measures empty any longer** — 6 `<caption>` of 8,111 recent nest inside another, and 6 inside an exhibit are owned by a `<supplementary-material>` rather than by the exhibit enclosing them, both counts being one eLife article (`PMC12143881`, the same one carrying every nested `<fig>` in the window) depositing its figure supplements that way. That is the shape earlier drafts asserted and the two previous draws could not find; the stack and the owner test are what keep those six legends off the enclosing figure, and being one publisher's house style it is a per-deposit property rather than a rate. The premise the half rests on still measures **full**, 6,938 / 6,938 carrying a direct-child `<caption>`, which is a measured result and not a symmetry with the `<label>` premise the same redraw broke. The back-filled window contributes nothing on either count, holding **0 `<caption>`**: its zeroes are an absent denominator, not a second measurement. A usurping title is **dropped, not relocated**: it was never a heading, and bmlib models none of the containers that carry one — though only the `<title>`, since a section-level `<caption>`'s `<p>` children never enter the exhibit branch and still reach the section's prose (issue #137). The **exhibit test the parent rule replaced on the section branch has to stay on the abstract branch**: JATS admits a `<fig>` and a `<table-wrap>` in an `<abstract>`, and the old `if in_figure or in_table_wrap:` opening the whole `<title>` arm swallowed every title inside one, so without an explicit guard a `<table-wrap-foot><fn-group><title>` there flushes the pending abstract section and installs itself as the next heading — #125 one branch over, and the worse half of it, since `abstract_sections` is rendered into the HTML `FullTextService` caches while `body_sections` reaches no bmlib path at all. That population was measured **empty** — 44 exhibits inside an `<abstract>`, none carrying a `<title>` — over the two 300-article draws that #138 replaced, and the sampler carries no counter for it, so nothing in the repo re-derives the 44 and the next reader must re-measure rather than trust it. The sampler's walk now **skips** `<sub-article>` and `<response>` exactly as the parser suppresses them, so every counter is a suppressed-region-excluding count; what the scoping removed is kept per row in `unscoped` rather than discarded, non-empty for the 29 of 997 recent articles carrying a region (145 regions) and for none of the back-filled window. #138 was scope *and* redraw, since scoping alone would have left the corpora unre-derivable.
...
And **a name that arrives undivided is kept undivided** (#120, #140). `<collab>` names a group and `<string-name>` a person the depositor did not split; each gets a field of its own on `JATSAuthorInfo` rather than being folded into `surname`, which is sorted and de-duplicated on and where an organisation is indistinguishable from a person. Verbatim in both cases, because splitting *"Ahmed Al-Rashid"* is a decision about particles, multi-word surnames and name order — assumed rather than measured, and undetectable by the caller once it is sitting in `surname`. `full_name` prefers a structured name over both, since a `<contrib>` carrying a `<name>` *and* a `<collab>` is *"Smith, on behalf of the Y Group"*. Four things the extraction needed beyond the two fields, three of them the same shapes this module has already been caught by. **`<contrib>` is a stack and `in_contrib`/`current_author` are derived from it**, because a `<collab>` may carry a `<contrib-group>` of the collaboration's own members: held as one slot each member overwrote the consortium's builder and its close cleared the flag, which is #115 one element family over — and a *non-author* `<contrib>` pushes a `None` frame, since skipping the push lets an editor's end tag pop the author's own, and reading the nearest builder rather than the top of the stack writes that editor's surname into the consortium. **A contributor is listed where its `<contrib>` opened**, the exhibits' reserve-and-fill, or a consortium is listed behind the members it encloses; the reservation is *given back* when the `<contrib>` names nobody, so an unfilled slot keeps meaning "never closed" and cannot make the audit ERROR on the well-formed `<anonymous/>`. **Both undivided spellings accumulate a buffer and merge it back, except inside a `<contrib>`** — accumulating so the close reads its own text and not the ancestor's, inline so a `<mixed-citation>` printing one keeps that name in the citation string it renders, and body prose keeps a name printed in it. But the nearest accumulating ancestor of a *roster member* is the enclosing `<collab>`, so an unconditional merge appended every member to the consortium's own name — *"the INHERIT Trial GroupJane Q SmithAhmed Al-Rashid"*, silently, in the exact shape #120 exists to collect. `_UNDIVIDED_NAME_ELEMENTS` refuses the merge while any `<contrib>` is open: the owner test in its usual form, the `<contrib>` owning the name. `collab` had to join `string-name` in `_INLINE_ELEMENTS` at the same time — it was accumulating and not inline, so every consortium-authored reference lost its author from the rendered citation, which is the defect the `string-name` entry beside it was added to avoid, one line up in the same set. **The `<contrib>` field is filled only when no structured name arrived**, since JATS lets `<string-name>` carry `<surname>` and `<given-names>` children and the buffer then holds only the punctuation between them; testing `surname` alone short-circuits, so the guard reads `given_names` too. **The reference branch is gated on the whole citation**, exactly as `<collab>`'s is: JATS admits either spelling as a direct child of `<mixed-citation>`, and the narrower `in_ref_person_group` dropped a cited name that was sitting in the markup. There the same divided shape needs a *flush* rather than a refusal — appending the buffer put a bare `","` in `references[].authors` ahead of the name, and only `</name>` and `</person-group>` finish a pending cited author, neither of which closes between two adjacent `<string-name>`, so the first of two collapsed onto the second. A `<contrib>` from which no name could be read is **counted and reported once per article at WARNING** from `_audit_parse`, which is where `rejected_spans` settled the same question (#129): a per-`<contrib>` DEBUG was too quiet to be #120's other half, was 200 identical lines on an author list of 200 `<xref>`-only contribs, and — emitted from `endElement` — named an article whose `<article-id>` had not been read yet. It says *bmlib read no name*, never *the document carried none*: `<on-behalf-of>` is a fourth spelling, JATS-legal and unextracted (#144), and an article naming its only contributor that way reached the **quiet** branch of the zero-author detector until that spelling was added to `front_contributor_name_count`. Counting is not parsing, and the counter must not let the detector conclude past it. `JATSAuthorInfo.is_named` is where "did any spelling arrive?" is defined — on the public type, so `build()` is a gate and not a second definition, and deliberately **not** a raising `__post_init__`, which is #129 exactly: an exception out of a SAX callback into `service.py`'s tier-level `except Exception` costs the whole article. The rule is spec-driven rather than drawn, and the #138 redraw has now **run** section 11's counters, scoped so a peer-review `<sub-article>`'s reviewers no longer inflate what the parser never sees: across 12,659 `<contrib>` in the two committed corpora, `<collab>` names 14 contributors (all in the recent window: 0.18% [0.11-0.30] of *that* window's 7,798) and **`<string-name>` names none at all** — 0 of 12,659, upper bound 0.03%, a measured absence rather than an omission, since the vocabulary is open and an unforeseen spelling prints as itself. Beside them: **0** nested `<contrib>`, **0** `<collab>` carrying a roster, and 2 of 997 recent articles naming every contributor undivided. Those last two were 20 and 1 in the draw this one replaced, and `<on-behalf-of>` 1 against 0 here — three populations this window does not reach rather than three it refutes, which is why the rules stand on the spec and not on a count. #120's own 34 of 1,025 articles, from the #118 review rather than a committed corpus, counted `<contrib>` elements carrying **no `<surname>`** — a set both spellings share — so it stays a rate for neither of them alone; quote the per-spelling counts above instead. Section 11's vocabulary is deliberately **open** — every non-excluded child of a `<contrib>` is counted under its own name, and only the annotation consults a set — because counted against a closed list an unforeseen spelling falls into `(none)` and is reported as a contributor naming nobody, which is #121's mis-certification inside the instrument built to detect the next #120.

And **a `<mixed-citation>` is mixed content, so every descendant's text is its own** (#146). `JATSReferenceInfo.citation` is built from that element's buffer, and a child accumulating a buffer without merging it back has its text *taken and not returned* — `<person-group>`, `<article-title>`, `<source>`, `<year>`, `<volume>`, `<issue>`, `<fpage>`, `<lpage>` and `<pub-id>` all do, which is the whole of a standard NLM deposit, so the rendered string was the punctuation between them: `'. . . ;():-. doi: .'`. **The rule is a property of the context, not of the element**, which is why `_INLINE_ELEMENTS` — right for `<collab>` and `<string-name>`, which carry a name wherever they appear — cannot serve: an `<article-title>` in `<article-meta>` is the article's own title, and merging it unconditionally appends it to whatever buffer is open. So it is an **ancestor** test, not the parent test the module usually makes, because mixed content is inherited down the whole subtree and each merge composes into the one above. `<element-citation>` is excluded and leaves `citation` **empty** — element-only content authored no string, the whitespace between its children is insignificant, and assembling one is a citation-style decision `formatted_citation` already makes — but **excluding it from the merge was necessary and not sufficient**, which is the review's finding rather than the issue's. A child bmlib does not *accumulate* never withheld a buffer to begin with: its characters go straight to whatever is open, so a routine book deposit's `<edition>`/`<publisher-loc>`/`<publisher-name>` gave `'3rd edAmsterdamElsevier'` — the run-together word the exclusion exists to avoid, documented in six places as the empty string. So the close arm writes `citation` for `<mixed-citation>` **only**, which also settles a `<ref>` carrying both spellings (legal as bare siblings and inside `<citation-alternatives>`), where the unconditional write was last-writer-wins and an `<element-citation>` second wiped the string the publisher did typeset. Several citation elements in one `<ref>` is **#149, settled by measurement rather than preference**: 216 such references in 21 of 880 local articles, **0 using `<citation-alternatives>`** — so never one reference deposited twice, but either several works under one bibliography number (149, RSC's `(a)`/`(b)`) or one reference *split*, its URL tail given its own element (61). The second shape is what rules out one `JATSReferenceInfo` per part, since it would split a work into a work plus a bare URL; so a `<ref>` stays one reference and the parts are **joined with nothing between them**, the inter-element character data measuring empty in **586 of 586**, each part kept raw and the whole normalised once at `</ref>` — "strip once, at the outermost call", which keeps the space before `(b)` without inventing one before `, [Online]`. **The structured fields are first-wins**, every field arm being gated on `in_ref_citation` so leaving it unset for later parts is the whole of it: scalars stop being last-wins and `authors` stops accumulating, which had welded 40 authors from different papers into one byline (86 references drew fields from more than one part). And **a part's `<label>` is no longer the reference's number** — that branch alone was gated on the ambient `in_ref`, the routing #116 established is wrong, so RSC's markers overwrote it; measured at 158 references in 14 articles with **nought** overwriting a real label, so the whole population was an invented number, which is #116's symptom exactly. **The blast radius is measured, not reasoned, and the first account had it backwards**: diffing the branch against `main` over 880 local PMC articles / 20,770 references moves `citation` for 4,499 (21.7%) in 191 articles — 3,541 rebuilt by the merge and **958 in 84 articles emptied**, those having been carrying an element-only leak in a public field — **`authors` for 502 in 14 articles**, and rendered HTML for 576 in 23 — so a *structured* field moves, because `<surname>`/`<given-names>` are gated on `in_ref_person_group` and a cited `<string-name>` outside one had no arm fire at all. That path needed the `<collab>` and `<string-name>` reference arms **normalised rather than end-stripped**, a wrapped name having arrived as the literal `'J.\nTan'` in a public list and in the cached HTML. The strict-ancestor slice — which stops the citation merging into whatever encloses the `<ref>` — is **prospective and the comment says so**: at this revision no handler reads that buffer (nothing outside `_TEXT_ACCUMULATING` takes `text`), dropping the slice survives the whole suite, and three document shapes parse identically without it. That premise is now **mechanised rather than asserted** (#151), because it is a property of a 500-line method written in one helper's docstring 300 lines away and #142 is exactly the change that breaks it: `TestOnlyAnAccumulatingElementReadsTheBuffer` walks `endElement`'s arms with `ast` and fails on one reading the popped buffer for an element outside `_TEXT_ACCUMULATING` — the `TestTheAuditNetIsComplete` precedent, *a rule enforced by prose is not enforced*. Four rules on it. **The net is keyed on the buffer, not on identifiers**: `self.current_text` and `self._pop_text_buffer()` are watched beside `text`/`normalized_text`/`element_text`, because for a non-accumulating element the preamble's `element_text = self.current_text` makes the two the same value — keyed on the three locals alone, #142's own arm written in the `current_text` spelling passed the whole suite green, so which of two synonymous forms the implementer typed decided whether the guard fired. **Plumbing is what a statement *does*, not what it binds**: a read is passed over only when it stands under no guard, binds a buffer name, *and* hands the buffer to no method on the handler — exempting on the binding alone let `text = self._collab_child(name, text)` beside the preamble read the buffer for every element in silence, which was the one shape inside the method producing no finding at all. **Every other outcome fails closed**: a guard it cannot read is a finding rather than a skipped read, a read no `name` test constrains is reachable for every element, the verdict is *containment* and never overlap — the likeliest breakage being an existing arm gaining an element rather than a new arm appearing — and it raises rather than answering "no reads" for a class or method it cannot find, since that answer would turn four tests green at once. And **a control is judged against its own accumulating set, never the parser's**: asserting `<institution>` does not accumulate is asserting what #142 is entitled to change, and seven controls would otherwise fail for the opposite of the reason they were written. The positive control carries the **whole** inventory of arms that consume the buffer — nineteen elements, read as a floor — because six names while the walk saw nineteen left thirty-eight of the fifty-three reads it is built from free to leave without a word, and a *partial* extraction of one arm into a helper is a likelier refactor than the wholesale kind; a floor rather than an equality so #142's legitimate arm needs no edit to let it through. The walk covers `endElement`'s own body, so a read that moves into a helper is out of reach — that inventory is what notices it going. Beside it, `element_stack[:-1]` is a strict-ancestor slice **only because `element_stack.pop()` sits at the end of `endElement`**, which was stated at neither site; measuring that corrected the issue's own claim, since `_inside_mixed_citation` is evaluated inside `_pop_text_buffer`'s argument and so only a pop moved above the *buffer* pop reaches the slice — 58 tests redden for a pop before the handler arms, 65 for one above that call. The slice's guard is that seven-test difference: three of `TestAMixedCitationKeepsTheTextItPrints`' six and **four in `TestARefCarryingSeveralCitationsKeepsThemAll`**, the majority, which the first draft of the comment omitted. Two things came out of walking the other paths the merge rule reaches. A `<xref>` to a figure or table is **replaced** by a `[text](#rid)` link rather than merged, and nothing pinned that — dropping the suppression emitted `Figure 1[Figure 1](#f1)` into body prose and passed the full suite. And the same taken-and-not-returned shape lost a `<tex-math>` from the prose around it and a `<disp-formula>` from the article outright, which is #147: delimiting LaTeX in prose is a decision, and `<alternatives>` may hold two encodings of one formula, so it was not one more member of a set. **That is fixed, and the citation path is not the exception this paragraph used to name.** `_FORMULA_PARTS` is subtracted from the merge *after* the ancestor test, so a `<tex-math>` inside a `<mixed-citation>` is rendered by the formula arm exactly as it is anywhere else — `'Bare $x+y$ deposit.'`, not the raw document, and one rendition where an `<alternatives>` pair would have printed two. The measured **0** of 10,671 `<mixed-citation>` across 227 articles and 0 in the local corpus therefore sizes an *unexercised* path rather than a live gap, which is what it was already doing while the sentence around it said otherwise. Finally, `_INLINE_ELEMENTS`' `collab`/`string-name` entries are now **subsumed** for every measured shape: after #146, deleting both passes the whole suite where before it failed two tests, so what they still stand for is a name printed outside a citation — an unmeasured population, pinned by `TestAnUndividedNameInProseStaysInTheProse` so the membership cannot go vacuous again.

And **a formula emits one chosen rendition, at the formula element** (#147). `<tex-math>` accumulates and is not inline, so an `<inline-formula>` merged an empty buffer and the sentence rendered with a hole in it; `<disp-formula>` accumulates with *no handler*, so a display equation was popped and discarded whole — LaTeX, MathML and the `(1)` body prose cross-references. The obvious fix is refused by measurement: 1,087 formulas in the committed recent corpus and 188,473 across the `PMC012xxxxxx` package carry both encodings of one expression, so adding `<tex-math>` to `_INLINE_ELEMENTS` prints each twice. So the encodings never merge on their own — LaTeX wins where a `<tex-math>` arrived, and **the formula's own buffer serves otherwise**, which is what keeps the change small: MathML accumulates nothing, so its flattening is already in that buffer, and a MathML deposit bound to a prefix other than `mml` therefore keeps today's behaviour rather than depending on a literal prefix match the way #128 does. The encodings are held rather than resolved as they arrive because either may come first — 4,377 of that package's 188,473 both-encoding formulas are MathML-first, and **that is 37 of its 97,909 articles at ~118 apiece**, a house style rather than a rate, so a 997-article draw expects none and a zero must not be read as "the order never varies". **The LaTeX is a whole document**: 99.9% of 4,422 sampled deposits are `\documentclass` … `\begin{document}` …, every one of the 7,769 sampled *document-wrapped* deposits carries exactly one document pair — a count of the wrapped ones and not of all of them, the 3 bare expressions among the 4,422 carrying no pair at all — and 147 of 147 in two live-fetched Europe PMC articles, the rendition bmlib is fed. **The two markers are read independently**, since requiring both let a truncated deposit fall through to the bare-expression path and delimit its own preamble, which is both failures this rule exists to prevent in one string (0 unpaired measured, so severity and not frequency). 96.0% of the bodies already carry `$$…$$`, so the body is extracted and a pair added only where there is none — **except that a display pair on an *inline* formula is re-spelled `$…$`**: 98.6% of 20,251 inline `<tex-math>` bodies carry `$$…$$`, and an inline formula cannot genuinely be 98.6% display math, so that pair is the `minimal`-documentclass converter's artifact rather than a claim about context — left verbatim it rendered `'×'` as `'$$\times$$'` in a figure caption. The rule is one-directional, an inline pair on a display formula being under-styling where the reverse is broken markup, and a body carrying several delimited runs (`$a$ + $b$`) is left alone. Whether LaTeX should win at all for a *both-encoding inline* formula is #178: it replaces prose that was already correct in 20,046 formulas against the 205 it recovers. **A table cell is the path no buffer rule reaches**, since `characters()` fills it directly: 24,476 `<tex-math>` in 856 of the package's 97,909 articles sit in a `<td>`/`<th>`, and each pasted its preamble into the rendered table, so the cell is held back while a formula is open and takes the same one rendition. Routing is measured twice over: a `<disp-formula>` inside a `<p>` **merges** because emitted as its own paragraph it lands *ahead* of the paragraph it interrupts — and **how often that is depends on the rendition**, 116,623 of 150,598 (77.4%) over the package's archive bytes against 714 of 1,915 (37.3%) in the committed recent corpus and 201 of 654 (30.7%) in the served draw, so the two served measurements agree and a `<p>` is the *minority* parent on the bytes bmlib is fed, where it had been written down as the commonest shape (#164); one deposited as a block child becomes a paragraph, routed exactly as a `<p>` is through the shared `_append_prose`. Its `<label>` is read by the **parent** test (#116's rule — an ambient one takes a `<table-wrap-foot><fn>`'s marker) and printed **only where a number is what the reader reads**: merged into a sentence the bare number reads as a coefficient (`'as shown in eqn (2):2 τ = kn'`) and welds onto the previous formula (`'… + OH−2 Al3+ …'`), and a corruption is worse than a blank. **A cell is the exception, because a cell is a slot and not a sentence** — withholding it there was a regression, `characters()` having delivered the label to the cell before, and all 40 labelled display formulas measured in a cell (8 of 97,909 articles) are cells whose whole content is the number and the equation: PMC12164272's Table 2 is a reaction-number column the body prose cross-references. An image-only formula emits nothing, #162's rule — 140 of 1,915 display formulas hold nothing but a `<graphic>` once a `<label>` is set aside, which is the population the counter measures and not "a `<graphic>` and a `<label>`": at least 12 of the 140 carry no label. **Among several `<tex-math>` the first that renders wins**, `<alternatives>` holding alternative encodings of one expression, and the buffer is reached whenever none renders — testing the list for presence let an empty deposit suppress a good MathML flattening. Both populations measure 0. A rendition that reaches no section, caption or cell is **counted and reported once per article at WARNING** (`formulas_dropped`), the `rejected_spans` rule: 192 in 23 of 97,909 articles sit in an unsectioned `<back>`, and routing them is #177. Spacing follows the deposit — the module's own `_text_with_formatting` rule, a run's edge whitespace re-emitted outside its markers — except for a merged *block*, which has no spacing of its own and welds without one space either side. **Blast radius, and the first statement of it counted only what it could see.** "433 paragraphs gained, none lost, abstracts/tables/citations zero" was of an 880-article draw nobody can re-take, and *gained/lost* is structurally blind to a paragraph that changed in place — which is the commoner effect. Re-measured against `main` over the 880 articles of Europe PMC's named OA package `PMC10030002_PMC10040000.xml.gz`: prose moves in **34** articles, abstracts in **3**, `html_content` in **12**, and figure and table captions in **17** and **5** — captions being a public field the earlier statement named nowhere. Paragraphs: 1 gained, **0 lost**, **159 changed in place**. Tables moving is the *point* rather than a surprise, every one of those 12 being the LaTeX preamble leaving a rendered cell; a claim of zero there could never have been consistent with `tex_math_in_a_cell`'s 24,476 deposits in 856 articles two sentences earlier.
...
`scripts/sample_jats_exhibits.py` is the fifth live runner, and the evidence behind the parser's exhibit rules — **run it before changing `_ARCHIVAL_MIME_SUBTYPES`, `_ARCHIVAL_EXTENSIONS`, `_GRAPHIC_TRANSPARENT_WRAPPERS`, or the `<label>` parent test**. It answers eight questions with one walk: is a `<label>` a direct child of its exhibit (the parent rule's whole premise), what else carries one, do `<alternatives>` members declare `mime-subtype`, how are several `<graphic>` deposited and at which end the thumbnail sits, the same two questions for a `<table-wrap>`'s deposits (counted separately, #135, since the figure percentages are cited and widening their denominator would invalidate them), is a `<graphic>` ever owned by something other than its exhibit, how often a `<table-wrap>` carries a `<graphic>` and no `<table>` (#127) — or both, which is the rendition `to_html()` drops — and, as of #123/#125/#130, the same premise-and-owner pair for a `<caption>` plus what owns a `<title>` that a `<sec>` was open for. **Both exhibit sides are owner-scoped** — the table side since #135, the figure side since #164: `_owned` walks only through transparent wrappers, the way the parser routes, because unscoped a `<td>`'s cell image counts as the table's own deposit and made four of ten recent tables read as multi-deposit. The figure side kept the subtree walk longer because every share #117 cites was of it, on an argument the redrawn corpora refuted ("zero nested exhibits, every foreign owner a `<td>`"); scoping it moved `figures_multi_graphic` by 18 figures in 4 of 997 articles, and both readings are kept per row so the correction is derivable inside one draw. Because the sampler now measures *within* the parser's transparent-wrapper set, that set is the one thing here it must **not** be free to disagree on — pinned by `test_the_wrapper_set_is_identical_to_the_parsers`, where the judgement predicates below are pinned as differing. It shares the others' rules — the `_sampling` pacer and clamp, an unmeasured article entering no denominator, a population past `UNMEASURED_SHARE_ERROR_THRESHOLD` reporting ERROR and writing `*.unreportable.json`, a non-zero exit — and their prohibition: **it does not import the parser's predicates**, carrying its own thumbnail and archival tests, because a corpus labelled by the rule under test can only confirm that rule. `tests/test_jats_exhibit_sampler.py` asserts the two sets actually differ, which is what a future "deduplication" would break. One rule is its own: **the live sample is stratified by publication month**, because a single cursor walk from `*` returns a contiguous block of accessions — the first live run drew 120 articles of which 106 carried no exhibit at all, which is a property of one accession range and not a rate. **And the window is displaceable** (`--months-ago`), because the default live draw is the last two years and a population living in back-filled material reads as absent from it: #127's image-only tables measured 0 of 662 there and 11 of 93 — 11.8% [6.7-20.0] — in a draw ending 28 years back, though **neither draw is in the repo**, both having been replaced by #138, whose back-filled window carries no `<table-wrap>` at all. A displaced draw must name its own `-o`: the default path is the recent corpus and the journal follows it, so writing one there would replace that corpus under its own name, or pool the two windows into a rate describing neither. **The two committed corpora are no longer live draws** (#138): `--package` draws deterministically from a named PMC OA baseline package, so a reader re-derives the identifier list from `(packages, window, target, seed)` — all four recorded in the file — where a live window counted back from *today* named a sample nobody else could take (#132). **But the sample and the bytes come from different places, and that is the finding**: a package holds an *archive* rendition while `FullTextService` feeds the parser Europe PMC's `fullTextXML`, and the two disagree on cited populations — `last_is_thumb` differs in 156 of 300 compared articles, and where it differs the archive measures 0 against 781 served. The artifact records only disagreements, so that is the whole of what it supports: no archive total, and no single mechanism (`PMC12169732` deposits its own thumbnails under `specific-use` where Europe PMC re-labels them under `content-type`, and the two agree). So `--measure-europepmc` measures package-drawn identifiers from the served rendition, `window["rendition"]` records which, and `--compare-europepmc N` writes `tests/data/jats_exhibits.rendition.json`, the committed evidence for all of it. **Naming the artifact is necessary and not sufficient — check the rendition too.** The live path is kept for the rendition it measures, at the cost of a draw nobody can repeat. **And the walk shares the parser's scope**: `<sub-article>`/`<response>` regions are skipped exactly as the parser suppresses them, with what the scoping removed kept per row in `unscoped` so the correction is measurable rather than asserted — non-empty for the 29 of 997 recent articles carrying a region and for none of the back-filled window. **Six further guards came out of the review** (#165-#170), each a case of the instrument being trusted past what it had established: a package path is judged by opening the tarball and not by the gzip magic bytes, and the directory walk reaches the same depth as the tar walk (or one artifact read two ways names two different corpora); the guards on `DEFAULT_OUTPUT` **resolve** both sides, since raw `PurePath` equality let `-o "$PWD/…"` overwrite the committed corpus at exit 0; `measure_article` checks the root element, since a proxy's HTML error page at HTTP 200 parses cleanly into an all-zero row that enters every denominator; `compare_renditions` tells an unparseable served body from a failed fetch, the distinction `_measure_and_journal` already argued at length; `print_report`'s return value means what its docstring says, so a sample short of a counter generation reaches `*.unreportable.json` rather than the canonical name with `-1` inline, and `articles_where`/`sum_of` no longer read that sentinel as data in opposite directions; and the corpus holds only the rows its own header explains, rows outside this run's draw staying in the journal. `TestEveryCounterIsInAGeneration` is `TestTheAuditNetIsComplete` applied to the sentinel — **add a counter to `ArticleMeasurement`, add it to a generation** — because that rule had lived in prose. **That rule needed its other end too** (#164): `from_dict` splatted a hand-written list of generation tuples *beside* the registry rather than reading it, and `_FORMULA_ROUTING_COUNTERS` was registered and never added, so every row of both corpora loaded its three integer counters at 0 and section 14b printed `<tex-math> inside a <td>/<th>: 0  0.0%` over a population nothing had counted — on the population #147's live-corruption fix rests on. The registry is the single source now and `TestASentinelReachesEveryGenerationsCounters` checks it; a `Counter` field takes no sentinel (there is no negative dict), so a generation is detectable as absent through its *integer* counters alone. Beside it, **a section is gated on every counter it reads**, not on the generation it is named after — 14b divides by `tex_math`, one generation older, and reached `wilson()` with `n = -1` — and `articles_where` must not borrow `_as_count`, which flattens a `Counter` to 0: right for `measured`, and it made every `Counter`-backed population read as carried by no article, so the report printed `in 0 articles` over one the corpus holds in a hundred. Two more are nets rather than fixes and say so: an undated article is named on stderr rather than dropped in silence (0 of 220,485 today), and `article_year`'s **whole-document** scope — the one thing here not scoped to what the parser routes — is stated and measured at 0 of 3,385 region-carrying articles. **The figure-side graphic walk is owner-scoped as of #164**, as the table side has been since #135: the premise that the asymmetry cost nothing ("zero nested exhibits, every foreign owner a `<td>`") was refuted by the redrawn corpora, and re-measuring #117 on the served rendition put the cost at 18 of 2,676 multi-graphic figures and nothing else. Both readings are kept per row, because a redraw moves the sample and the served bytes with the walk — for this one it moved neither, the recent window's 997 identifiers and every unscoped counter coming back identical, which is what licenses reading the movement as the scoping's.

**Four guards came out of PR #180's review, each an instrument defect the fixes above did not reach.** A journal carries a `definitions` stamp (`_COUNTER_DEFINITIONS_VERSION`) beside `(source, rendition, draw)`, because #164 redefined four *first-generation* counters in place and every existing mechanism detects only an **absent** field — so a pre-#164 journal for the same package, window and seed resumed cleanly and pooled, printing `2,664`, neither reading, under a heading asserting the new one; bump it whenever an existing counter starts counting something else, which no test can detect for you. A section is gated on every counter it **reads** and on no others, now enforced by an AST walk (`TestEverySectionIsGatedOnEveryCounterItReads`) rather than by 14b's single hand-written scenario, with `_pct` failing closed on a sentinel as the backstop; the converse held too, section 4 having named #162's counter, which it does not read. `articles_where` **raises** on a type it does not know rather than scoring zero — the silent zero it was rewritten to remove, one type on, and `unscoped` is a plain `dict` that read as carried by 0 articles over 29 non-empty rows. And `_TRANSPARENT_WRAPPERS` is a **scope, not a judgement**: it was grouped with `_ARCHIVAL_HINTS`/`_THUMB_PATTERN`, which must differ from the parser's sets, while since #164 it decides every #117 share, so it is pinned as *identical* instead.
