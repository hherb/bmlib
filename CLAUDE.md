...
**Both committed corpora were redrawn for #138, and the redraw is what the figures below are of.** Each is 997 articles whose *identifiers* come deterministically from a named public artifact — `oa_comm_xml.PMC012xxxxxx.baseline.2025-06-26.tar.gz` for 2023-2025 and `oa_comm_xml.PMC002xxxxxx.baseline.2025-06-26.tar.gz` for 1996-1998, at `seed 0` — while the *bytes measured* are Europe PMC's `fullTextXML`, the rendition `FullTextService` feeds the parser. The two halves are separate because the two renditions disagree on the cited populations and not merely on detail: `last_is_thumb` **differs in 153 of 294 compared articles, and where it differs the archive measures 0 against 641 served** — so a corpus drawn *and* measured from a baseline package would put #117's whole ranking rule out of reach and read it as dead code. **Read that at that scope and no further**, which is this repo's own rule about a count being of what you looked for, got wrong first by the person writing the rule down: `tests/data/jats_exhibits.rendition.json` records a field only where the renditions disagree, so an agreeing article never appears and the archive's total over all 294 is not derivable from it. Nor is there one mechanism to name — an early draft said the archive deposits a bare `…-g001` per figure where Europe PMC synthesises an `.jpg`/`.gif` pair, true of a spot-checked article and false in general, since `PMC12169732` deposits its own four thumbnails as `specific-use="thumbnail"` where Europe PMC re-labels them `content-type="thumb"` and both measure four. Every figure below moves for **three** reasons at once — a different sample, a different rendition, and a scoped walk — so **no movement here may be attributed to any one of them**, the scoping least of all, since it is the only one whose effect the corpus records (`unscoped`).

One rule measures **empty**, and the comments say so rather than implying a population: of **6,385** `<graphic>` inside an `<alternatives>` across the two draws, **none declares a `mime-subtype` and none is archival by either test**, so the whole `ARCHIVAL` rank is unreached. It is kept because what it prevents is silent and permanent, and "no instance" is not "cannot happen"; the `<p>` member of the transparent set is *not* defensive, since without it a figure whose graphic sits in prose flow loses it outright. **The second one moved twice, and matters more than it did.** A vanished 276-article draw found exactly one `<graphic>` owned by a non-exhibit inside an exhibit, which read as a population of one; the recent committed draw finds **102, in 12 of 997 articles, over five owners** — `<td>` 48, `<inline-formula>` 42, `<chem-struct>` 9, `<disp-formula>` 2, `<th>` 1 — and since #127 gave `JATSTableInfo` a `graphic_url`, relaxing ownership lands a cell decoration in it as if it were the table's own rendition. So that rule is measured as load-bearing, not carried against a hypothetical, and the five-owner spread is itself the argument for keeping the *listed* side short and everything else opaque. **The parent rule's premise, by contrast, is now VIOLATED, and this is the one claim the redraw overturned rather than moved.** Every draw before it measured the premise full — 2,033 / 2,033, 1,446 / 1,446, 365 / 365 — and none of those is re-derivable; on the served rendition **6,692 recent exhibits carry a direct-child `<label>` against 6,699 carrying one anywhere**, so 7 exhibits in 4 articles (`PMC12011025`, `PMC12092211`, `PMC12120668`, `PMC12174859`) carry their label only indirectly and lose it, `to_html` then substituting `Figure {i + 1}` — #116's own invented-number symptom, reached from the other direction. Do not restate the rule as one that "cannot lose a label"; it wants a decision about a fallback, not a prose repair. It is still the better of the two rules, and by a wide margin: a depth counter would *mis-assign* **607 labels in 101 of these 997 articles** — `<fn>` 367, `<list-item>` 222 and `<disp-formula>` 18, the last a container no enumeration written for #116 named — against 7 this one omits, and a corruption is worse than a blank. The back-filled window is where the premise still measures full, 627 / 627, all `<fig>`. Re-measuring #117 on the two draws gives **56.7%** [55.2-58.1] of 4,466 recent figures carrying several graphics and **55.9%** [54.5-57.4] ending on a thumbnail, and **44.0%** on both counts for 627 back-filled ones, against the 58.0% / 52.9% of the original 225-article survey and the 49.9% / 49.5% of the vanished 276-article draw — and **0%** depositing a thumbnail first in every one of them.

And **a `<title>` and a `<caption>` are routed by their owner too** (issues #125, #130, #123), which is the same rule one element further out. `<sec>` is far from the only element carrying a `<title>`: `<fn-group>` is modelled `(label?, title?, (fn|p)+)`, and `<ref-list>`, `<glossary>`, `<app>`, `<boxed-text>` and every `<caption>` carry one, so asking only "is a section open?" let any of them rename it — leaving not a blank but a heading the publisher never wrote, which is why it survived. **Measured, and not small**: 409 titles in 100 of 997 recent articles (10.0% [8.3-12.1]), owned by a `<caption>` (391, in 87 articles), a **`<def-list>`** (15, in 15) and an `<fn-group>` (3, in 1) — what owns that `<caption>` is not recorded, the sampler counting the `<title>`'s immediate parent alone. The `<def-list>` is the parent test's whole argument restated: every redraw has turned up an owner neither issue names, the previous back-filled draw's being a **`<list>`**, and no enumeration written from #125 and #130 would have held either. #125's own `<fn-group>` shape is in *this* draw, where it was in neither of the last two, and also reproduces on eLife's PMC8754430; 3 titles in 1 article is a floor for that shape rather than a rate. The redrawn back-filled window carries no renaming title at all, holding no `<caption>` whatever, so the whole population now rests on the recent draw. A `<caption>`'s prose is a **stack of owners**: held as a stored boolean, the inner `</caption>` truncated the enclosing caption at the point the inner one ended, and a depth fixes only that half, since the inner legend's owner is not an exhibit bmlib models. **Both of that half's populations measure empty** — no `<caption>` nests inside another (0 of 7,820 recent) and every `<caption>` inside an exhibit is owned by that exhibit, in the recent draw and in the seven-article Swift corpus — while the premise it rests on measures **full**, 6,709 / 6,709 carrying a direct-child `<caption>`, which is a measured result and not a symmetry with the `<label>` premise the same redraw broke. The back-filled window contributes nothing on either count, holding **0 `<caption>`**: its zeroes are an absent denominator, not a second measurement. A usurping title is **dropped, not relocated**: it was never a heading, and bmlib models none of the containers that carry one — though only the `<title>`, since a section-level `<caption>`'s `<p>` children never enter the exhibit branch and still reach the section's prose (issue #137). The **exhibit test the parent rule replaced on the section branch has to stay on the abstract branch**: JATS admits a `<fig>` and a `<table-wrap>` in an `<abstract>`, and the old `if in_figure or in_table_wrap:` opening the whole `<title>` arm swallowed every title inside one, so without an explicit guard a `<table-wrap-foot><fn-group><title>` there flushes the pending abstract section and installs itself as the next heading — #125 one branch over, and the worse half of it, since `abstract_sections` is rendered into the HTML `FullTextService` caches while `body_sections` reaches no bmlib path at all. That population was measured **empty** — 44 exhibits inside an `<abstract>`, none carrying a `<title>` — over the two 300-article draws that #138 replaced, and the sampler carries no counter for it, so nothing in the repo re-derives the 44 and the next reader must re-measure rather than trust it. The sampler's walk now **skips** `<sub-article>` and `<response>` exactly as the parser suppresses them, so every counter is a suppressed-region-excluding count; what the scoping removed is kept per row in `unscoped` rather than discarded, non-empty for the 25 of 997 recent articles carrying a region (141 regions) and for none of the back-filled window. #138 was scope *and* redraw, since scoping alone would have left the corpora unre-derivable.
