...
One rule measures **empty**, and the comments say so rather than implying a population: of **7,055** `<graphic>` inside an `<alternatives>` across the two draws, **none declares a `mime-subtype` and none is archival by either test**, so the whole `ARCHIVAL` rank is unreached. It is kept because what it prevents is silent and permanent, and "no instance" is not "cannot happen"; the `<p>` member of the transparent set is *not* defensive, since without it a figure whose graphic sits in prose flow loses it outright. **The second one moved twice, and matters more than it did.** A vanished 276-article draw found exactly one `<graphic>` owned by a non-exhibit inside an exhibit, which read as a population of one; the recent committed draw finds **153, in 12 of 997 articles, over three owners** — `<td>` 82 (8 articles), `<inline-formula>` 69 (3), `<disp-formula>` 2 (1) — and since #127 gave `JATSTableInfo` a `graphic_url`, relaxing ownership lands a cell decoration in it as if it were the table's own rendition. So that rule is measured as load-bearing, not carried against a hypothetical, and the owner spread is itself the argument for keeping the *listed* side short and everything else opaque: the previous draw of this same window turned up `<chem-struct>` and `<th>` as well, which this one does not, so the owners are drawn from rather than fixed. **The parent rule's premise, by contrast, is now VIOLATED, and this is the one claim the redraw overturned rather than moved.** Every draw before it measured the premise full — 2,033 / 2,033, 1,446 / 1,446, 365 / 365 — and none of those is re-derivable; on the served rendition **6,937 recent exhibits carry a direct-child `<label>` against 6,944 carrying one anywhere**, so 7 exhibits in 7 articles (`PMC12011025`, `PMC12111618`, `PMC12115352`, `PMC12149983`, `PMC12154067`, `PMC12159547`, `PMC12177175`) carry their label only indirectly and lose it, `to_html` then substituting `Figure {i + 1}` — #116's own invented-number symptom, reached from the other direction. Do not restate the rule as one that "cannot lose a label"; it wants a decision about a fallback, not a prose repair. It is still the better of the two rules, and by a wide margin: a depth counter would *mis-assign* **561 labels in 95 of these 997 articles** — `<fn>` 330, `<list-item>` 225 and `<supplementary-material>` 6, the last a container no enumeration written for #116 named — against 7 this one omits, and a corruption is worse than a blank. The back-filled window is where the premise still measures full, 627 / 627, all `<fig>`. Re-measuring #117 on the two draws gives **58.1%** [56.7-59.6] of 4,602 recent figures carrying several graphics and **57.3%** [55.9-58.8] ending on a thumbnail, and **44.0%** on both counts for 627 back-filled ones, against the 58.0% / 52.9% of the original 225-article survey and the 49.9% / 49.5% of the vanished 276-article draw — and **0%** depositing a thumbnail first in every one of them.
