...
Every rule above is a rule about *routing*, and each was found from outside bmlib after the fact. So **the parse is audited when it ends** (issue #134, `_parse_audit.py`): `_run_parser()` — the one place `parse`, `to_html` and `parse_with_html` funnel through — reads the handler's stacks, depths and flags into a frozen `ParseUnwindState` and logs one ERROR per imbalance, each naming what the imbalance *cost*. It is a **net, not an input check**: expat rejects an unbalanced *document*, so nothing a publisher deposits can reach these predicates and every line is a claim that bmlib itself is wrong — which is what fixes the level at ERROR. Nothing raises, since a partial article reported loudly beats no article. Three rules on it. **Every field defaults to its clean value**, which is what lets a test name only the imbalance it is about, and why `excess_text_buffers` counts the excess and not the depth — `text_stack` always holds one buffer, so the raw length would read every clean parse as broken. **Add a stack or a flag to the handler, add it here**; an incomplete net is what lets the next one hide, which is why the audit is its own module rather than a helper inside a 2,000-line parser. And **a predicate must be measured against well-formed input before it ships**: `current_abstract_text` is deliberately excluded because `</abstract>` flushes without clearing, so it is non-empty at the end of every article carrying an abstract. What caught that is the autouse `parser_log` fixture in `tests/test_jats_parser.py`, which fails any test in that module whose parse emits an ERROR — making all 202 existing fixtures a false-positive check without being written as one, and confirmed to have teeth by two mutants that failed 192 and 86 tests. Beside it, **a parse that yields no authors reports which kind it is** (#121): `front_surname_count` is gated on `in_front`, a structural fact, and never on `in_contrib`, which is set only once `_is_author_contrib` has said yes — the routing decision #111 got wrong, so a counter keyed on it goes to zero in exactly the situation it exists to detect. That one is **WARNING and not ERROR**, because it can fire on a document bmlib parsed correctly (`PMC12803704`, genuinely author-less and still carrying `<front>` surnames, 1 of 1,025 measured), and the audit's net depends on ERROR meaning only "bmlib is wrong". Finally, **a value that will not parse costs its own cell and nothing more** (#129): `colspan` is CDATA, and a bare `int()` raised out of the SAX callback into `service.py`'s tier-level `except Exception`, so one bad attribute lost the article and the chain reported it as unavailable from that source.
