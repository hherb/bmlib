...
Beside it, **a parse that yields no authors reports which kind it is** (#121): `front_contributor_name_count` is gated on `in_front`, a structural fact, and never on `in_contrib`, which is set only once `_is_author_contrib` has said yes — the routing decision #111 got wrong, so a counter keyed on it goes to zero in exactly the situation it exists to detect. That one is **WARNING and not ERROR**, because it can fire on a document bmlib parsed correctly (`PMC12803704`, genuinely author-less and still carrying `<front>` surnames, 1 of 1,025 measured during the Swift port and not reproducible from a committed corpus), and the audit's net depends on ERROR meaning only "bmlib is wrong". **It counts every JATS spelling of a contributor's name** — `<surname>`, `<string-name>` and `<collab>` — because a `<contrib>` is named with `(name | string-name | collab | …)` while bmlib extracts only `<name>`: counting surnames alone, a `<string-name>`-only article (100% of its authors lost, #140) and a `<collab>`-only one (#120) were both certified *genuinely author-less*, which is the exact silence #121 exists to end. Counting is not parsing, and the quiet branch now reports the evidence it has rather than that conclusion.
