...
/// Open an in-memory SQLite database with foreign keys on.
///
/// The equivalent of Python's `connect_sqlite(":memory:")`.
///
/// # Errors
///
...
///
/// Mirrors `connect_sqlite`: WAL and foreign keys default on down there, and
/// WAL is skipped for an in-memory database.
///
/// # Errors
///
/// If the parent directory cannot be created or the connection cannot be
/// opened.
pub fn open_path(path: &Path, wal_mode: bool, foreign_keys: bool) -> Result<Connection> {
    if let Some(parent) = path.parent() {
        if !parent.as_os_str().is_empty() {
            std::fs::create_dir_all(parent).map_err(|e| DbError::Backend(e.to_string()))?;
        }
    }
...
impl Db for Connection {
    ...
    ///
    /// The first place the port diverges from Python's `sqlite3`, which
    /// auto-begins before DML and leaves a write pending until an explicit
    /// `commit()`.
...
impl Db for Savepoint<'_> {
    ...
    /// the commit" — is what lets a batch loop wrap many `transaction()`-using
    /// calls in one outer block and pay a single commit.
    fn commit(self: Box<Self>) -> Result<()> {
