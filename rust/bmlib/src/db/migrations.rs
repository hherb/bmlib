...
//!
//! A port of `db/migrations.py`. The only signature change is that Python's
...
use crate::db::operations::{create_tables, execute, fetch_all, table_exists};
...
///
/// Each migration and its `schema_version` row are written in one
/// [`transaction`], so a migration that fails leaves neither its DDL nor a
/// version row claiming it succeeded.
///
/// # Errors
///
/// Whatever the first failing migration or driver reports.
