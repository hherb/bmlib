...
//!
//! The Python contract this reproduces, from `db/transactions.py`: entering a
//! block while another is open runs the inner one inside a `SAVEPOINT`; on
//! success the savepoint is released and **no commit is issued**, because
//! whoever opened the outermost block owns the commit; on failure only the
//! inner block's writes are rolled back.
//!
...
///
/// ```
/// use bmlib::db::{open_memory, transaction, execute, create_tables};
/// use bmlib::db::Db;
/// use bmlib::params;
///
/// fn store(db: &mut dyn Db, v: &str) -> bmlib::db::Result<()> {
///     transaction(db, |tx| {
...
///
/// ```compile_fail
/// use bmlib::db::{open_memory, transaction, params, execute};
///
/// let mut conn = open_memory().unwrap();
...
/// # Errors
///
/// Whatever `f` returns, or a driver failure from opening or committing.
...
///
/// Any module ported above `db/` has its own error, and [`transaction`]'s
/// fixed `DbError` closure leaves `?` inside the block with nothing to convert
/// through.
///
...
/// Measured: the generic version needed an `Ok::<_, DbError>` annotation in
/// the migration runner and at five call sites in the tests. Two entry points
/// cost one name; one generic entry point cost an annotation nearly
/// everywhere.
///
/// # Errors
///
