...
//! Pure-function query helpers over a [`Db`].
//!
...
use crate::db::backend::{adapt_sql, Dialect};
use crate::db::error::Result;
use crate::db::split::split_sql_statements;
...
/// # Errors
///
/// The first driver failure; statements already run are not undone, which is
/// why callers wanting atomicity wrap this in [`crate::db::transaction`].
...
/// surrounding [`crate::db::transaction`]. Both backends support transactional
...
///
/// **Deliberate divergence from the Python port:** Python splits for SQLite
...
/// [`split_sql_statements`] does not know PostgreSQL's dollar-quoted bodies
/// (`$$ … $$`), which bmlib's schema does not use but a `CREATE FUNCTION`
/// would.
///
/// # Errors
///
/// Whatever the driver reports for the first failing statement.
