...
//! The error type for `db/`, and for anything ported above it.
//!
//! Python signals failure by raising; every `bmlib.db` helper lets the
//! driver's exception propagate, and `transaction()` catches `Exception` to
//! decide rollback. Rust needs one concrete type that can carry both a driver
//! failure and a *caller's* failure, because the caller's `Err` is what tells
//! [`crate::db::transaction`] to roll back — the role `raise
//! RuntimeError("boom")` plays in the Python tests.
...
impl std::error::Error for DbError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            DbError::Abort(e) => Some(&**e),
            DbError::Backend(_) | DbError::Column(_) => None,
        }
    }
}
